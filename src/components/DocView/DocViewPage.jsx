      ...
      <header className="screen-head">
        <div>
          ...
          <p className="screen-sub">
            Compare the native Qyrova document with the linked Google Doc
            version.
          </p>
        </div>
        <div className="head-actions">
          ...
          {docMode === "googledoc" && (
            <button
              ...
              onClick={() =>
                window.open(preset.googleDocUrl, "_blank", "noopener")
              }
              title={
                preset?.googleDocUrl
                  ? ""
                  : "Link a Google Doc to this preset first"
              }
            >
              <ExternalLink size={16} />{" "}
              {canEdit ? "Edit in Google Docs" : "Open in Google Docs"}
            </button>
          )}
        </div>
      </header>
      ...
      {!preset ? (
        ...
        <>
          <div className="alert alert-info">
            <Info size={18} />
            <span>
              Native Qyrova document. Hide fields, add a banner or extra
              content, and copy any <code>{"{{placeholder}}"}</code> to build a
              Google Doc template.
            </span>
          </div>
          <motion.div
        ...
        <div className="empty-state">
          ...
          <p className="form-hint">
            Create a Google Doc with the placeholders from the Native Version,
            then link it here.
          </p>
          {canEdit && (
            <button
        ...
        >
          <div className="doc-page">
            <iframe
              title={`${preset.name} Google Doc preview`}
              src={previewUrl}
              className="doc-frame"
            />
          </div>
          <p className="doc-hint">
            Linked Google Doc template.{" "}
            {canEdit && (
              <>
                Use <strong>Edit in Google Docs</strong> to change it.
              </>
            )}
          </p>
        </motion.div>
